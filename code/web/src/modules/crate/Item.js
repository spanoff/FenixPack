    ...
    this.props.create({ crateId })
      .then(response => {
        if (response.data.errors && response.data.errors.length > 0) {
          ...
          this.props.messageShow("Подписано успешно.");

          this.props.history.push(userRoutes.subscriptions.path)
        }
      })
      .catch(error => {
        this.props.messageShow("Некоторая ошибка в этом пакете. Пожалуйста, попробуйте еще раз.");
      })
      .then(() => {
        this.setState({
          isLoading: false
        ...
        <div style={{ padding: '1em 1.2em' }}>
          ...
          <p style={{ textAlign: 'center', marginTop: '1.5em', marginBottom: '1em' }}>
            <Button
              ...
              disabled={ isLoading }
            >
              <Icon size={1.2} style={{ color: white }}>add</Icon>Подписаться
            </Button>
          </p>
        </div>
      </Card>
