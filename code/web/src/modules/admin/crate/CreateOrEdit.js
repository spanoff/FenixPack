...
class CreateOrEdit extends Component {
  ...
  getCrate = (crateId) => {
    if (crateId > 0) {
      this.props.getCrateById(crateId)
        ...
        .catch(error => {
          this.props.messageShow("Возникла некоторая ошибка при наборе типов пакетов. Пожалуйста, попробуйте еще раз.");
        })
    }
  }
  ...
  onSubmit = (event) => {
    ...
    this.props.messageShow("Сохраняется пакет, подождите ...");
    ...
    this.props.crateCreateOrUpdate(this.state.crate)
      .then(response => {
        ...
        if (response.data.errors && response.data.errors.length > 0) {
          ...
          this.props.messageShow('Пакет удачно сохранен')

          this.props.history.push(admin.crateList.path)
        }
      })
      .catch(error => {
        this.props.messageShow("Произошла какая-то ошибка. Пожалуйста, попробуйте еще раз.");
  ...
  render() {
    return (
      <div>
        {/* SEO */}
        <Helmet>
          <title>Пакеты / Создать или редактировать - Администрирование  - ТМ Феникс</title>
        </Helmet>
          ...
          <Grid alignCenter={true} style={{ padding: '1em' }}>
            <GridCell>
              <H4 font="secondary" style={{ marginBottom: '1em', textAlign: 'center' }}>
                {this.props.match.params.id === undefined ? 'Создаем' : 'Изменяем'} пакет
              </H4>
