...
class List extends PureComponent {
  ...
  remove = (id) => {
    if (id > 0) {
      let check = confirm("Вы уверены, что хотите удалить этот пакет?");
          ...
          .then(response => {
            if (response.status === 200) {
              if (response.data.errors && response.data.errors.length > 0) {
                ...
                this.props.messageShow('Пакет успешно удален.')
  ...
  render() {
    ...
    return (
      <div>
        {/* SEO */}
        <Helmet>
          <title>Пакеты - Администрирование - ТМ Феникс</title>
        </Helmet>
              ...
              <table className="striped">
                ...
                <tbody>
                {
                  isLoading
                    ? <tr>
                        <td colSpan="6">
                          <Loading message="Загрузка пакетов"/>
                        </td>
                      </tr>
                    : list.length > 0
                      ...
                      : <tr>
                          <td colSpan="6">
                            <EmptyMessage message="Пакеты отсутствуют"/>
                          </td>
                        </tr>
                }
                </tbody>
