...
class Item extends PureComponent {
  ...
  onClickUnsubscribe = (id) => {
    let check = confirm("Вы действительно хотите отказаться от подписки на этот пакет?");
    ...
    if(check) {
      ...
      this.props.messageShow("Подписывание, пожалуйста, подождите ...");
      ...
      this.props.remove({id})
        .then(response => {
          if (response.data.errors && response.data.errors.length > 0) {
            ...
            this.props.messageShow("Отказ от подписки прошел успешно.");

            this.props.getListByUser()
          }
        })
        .catch(error => {
          this.props.messageShow("Возникла ошибка при оформлении подписки на этот пакет. Пожалуйста, попробуйте еще раз.");
        })
        .then(() => {
          this.setState({
            isLoading: false
  ...
  render() {
    ...
    return <Card style={{ width: "18em", backgroundColor: white }}>
        ...
        <div style={{ padding: "1em 1.2em" }}>
          ...
          <p style={{ color: grey2, marginTop: "1em", fontSize: "0.8em", textAlign: "center" }}>
            Подписка на {new Date(createdAt).toDateString()}
          </p>
        </div>
      </Card>;
  }
