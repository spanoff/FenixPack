...
class Item extends PureComponent {
  ...
  subscribe = () => {
    ...
    if(user.isAuthenticated) {
      ...
      messageShow('Подписывание, пожалуйста подождите...')
      ...
      createSubscription({crateId: crate.id})
        .then(response => {
          if (response.data.errors && response.data.errors.length > 0) {
            ...
            messageShow('Вы успешно подписаны.')

            onSuccessSubscription()
          }
        })
        .catch(() => {
          messageShow('При подписке на этот пакет произошла ошибка. Пожалуйста, попробуйте еще раз..')
      ...
      messageShow("Вам необходимо войти в систему, прежде чем вы сможете подписаться на пакет.");
